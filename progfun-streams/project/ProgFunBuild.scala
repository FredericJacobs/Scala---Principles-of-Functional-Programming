...
object ProgFunBuild extends Build {
  ...
  lazy val submissionProject = Project(id = "submission", base = file(Settings.submissionDirName)) settings(
    ...
    gradingCourseId := "progfun-003",// TODO (VJ) set default to 0
  ...
  val gradeSetting = grade <<= (scalaTestSubmission, styleCheckSubmission, apiKey, gradeProjectDetails, streams) mapR { (_, _, apiKeyR, projectDetailsR, s) =>
    ...
    apiKeyR match {
      case Value(apiKey) if (!apiKey.isEmpty) =>
        val apiKeyAugmented = apiKey + (if(projectDetails.courseId == "progfun-epfl-001") "-epfl" else "")
        ...
        if (!GradingFeedback.apiState.isEmpty && !Settings.offlineMode) {
          ...
          // TODO (VJ) This is done due to an urgency. We need a better solution.
          CourseraHttp.submitGrade(GradingFeedback.feedbackString(), scoreString, GradingFeedback.apiState, apiKeyAugmented, projectDetails, logOpt) match {
            case Failure(msgs) =>
