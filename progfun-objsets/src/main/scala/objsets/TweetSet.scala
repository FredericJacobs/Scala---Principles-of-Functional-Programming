...
abstract class TweetSet {
  ...
  // Answer: The filter method can be implemented here since all it does is call its helper method in both cases (Empty or NonEmpty)
   ...
   */
   ...
   // Answer: We implement it in the subclasses. If we have an empty set, "that" is the union. If not, we need additional parameters to go into the binary tree.
  ...
  // Answer: This should be implemented in subclasses. 
  ...
  /**
   * We add an isEmpty convenience method
   */
...
class NonEmpty(elem: Tweet, left: TweetSet, right: TweetSet) extends TweetSet {
  ...
  /**
   * We make a convenience method to find the most retweeted tweet of two tweets. 
   */
  
  def comparativeMostRetweeted(firstTweet: Tweet, secondTweet:Tweet) = {
    if (firstTweet.retweets > secondTweet.retweets) firstTweet 
      else secondTweet
  }
  
  // Returning a single tweet doesn't make much sense when equalities. But yeah, that's the signature.
  ...
  /** 
   *  We add a convenience method of our own, returns that a non empty set is not empty.
   */
